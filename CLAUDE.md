...
> 每位成员必须在 `docs/dev-journals/` 下维护自己的开发日志。
> 日志是**答辩素材的直接来源**，不写日志 = 答辩时没有材料可讲。

### 11.1 日志位置

```
docs/dev-journals/
├── README.md                  # 日志目录说明 + Agent 填写指南
├── 成员A-开发日志.md           # 成员A：误用检测 + 集成
├── 成员B-开发日志.md           # 成员B：数据采集 + 协议
├── 成员C-开发日志.md           # 成员C：异常检测 + 基线
└── 成员D-开发日志.md           # 成员D：GUI + 告警 + 测试
```
...
| 场景 | 要求 |
|------|------|
| **每天开发结束后** | 花 5-10 分钟填写当日进度、遇到的问题、Agent 协作记录 |
| **每修完一个 bug** | 记录问题现象→原因→解决方案（答辩核心素材） |
| **每完成一个拓展功能** | 记录技术决策（为什么选 A 不选 B）、性能对比数据 |
| **每参考一个开源项目** | 记录借鉴了哪一点、怎么落地的 |

### 11.3 七个必填板块

每个成员模板包含以下板块，每一天都必须填写（不填的写 "/"）：

```
1. 今日进度          — 做了什么、完成了什么
2. 遇到的问题与解决    — 现象→原因→方案（答辩核心）
3. Agent 协作记录     — AI 帮了什么、效率提升多少
4. 技术决策           — 选了哪个方案、为什么
5. 性能/测试数据       — 数字说话（答辩时直接用）
6. 参考项目借鉴        — 从开源项目中学了什么
7. 明日计划           — 明天要做什么
```

### 11.4 使用 Agent 填写日志（推荐方式）

日志不是手写负担——**让 Agent 帮你整理**。典型对话：

```
"读取 docs/dev-journals/成员A-开发日志.md，
在今天的 Day 2 板块下，帮我记录：
- 修复了 misuse_detector 对 (?i) 标志的处理 bug
- 遇到的问题是正则预编译后丢失了 re.IGNORECASE 标志
- 解决方法是识别内联标志后传给 re.compile 的 flags 参数
- Agent 帮我定位了 bug 的具体代码行
- 跑测试后 SQL 注入检测准确率从 70% 提升到 95%"
```
...
每个日志模板末尾有一个「**答辩素材汇总**」section，Day 7 必须填写：
...
这 5 项内容**直接决定了答辩 PPT 中你的个人贡献部分**。

### 11.6 验收标准（成员A 检查）

- [ ] 每个成员的日志文件存在且至少有一个 Day 的记录
- [ ] 「答辩素材汇总」section 全部填写完毕（Day 7 检查）
- [ ] 有至少 3 条「遇到的问题与解决」记录（体现踩坑能力）
- [ ] 有至少 2 条「Agent 协作记录」（体现 AI 工具使用能力）
- [ ] 有至少 1 条「参考项目借鉴」（体现调研能力）
- [ ] 有至少 2 条「性能/测试数据」（体现数据说话能力）
